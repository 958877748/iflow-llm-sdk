...
import type { CliAuthConfig, OAuthCredentials, Logger } from '../types';
...
export abstract class BaseCliAuth {
    ...
    constructor(
        protected config: CliAuthConfig,
        logger?: Logger
    ) {
        // 默认使用 console 作为 logger
        this.logger = logger || {
            trace: () => { },
            debug: console.debug,
            info: console.info,
    ...
    async loadCredentials(): Promise<OAuthCredentials | null> {
        ...
        try {
            if (!fs.existsSync(credentialPath)) {
                this.logger.debug(`[${this.config.name}] 凭证文件不存在: ${credentialPath}`);
                return null;
            }
            ...
            this.logger.info(`[${this.config.name}] 已加载凭证`);
            return processedCredentials;
        } catch (error) {
            this.logger.error(`[${this.config.name}] 加载凭证失败:`, error);
            return null;
        }
    }
    ...
    async ensureAuthenticated(): Promise<OAuthCredentials | null> {
        ...
        if (isExpired && credentials.refresh_token) {
            try {
                ...
                this.logger.info(`[${this.config.name}] 令牌已刷新`);
            } catch (error) {
                this.logger.error(`[${this.config.name}] 令牌刷新失败:`, error);
                return null;
            }
        }
        return credentials;
    ...
    protected saveCredentials(credentials: Partial<OAuthCredentials>): void {
        ...
        if (fs.existsSync(credentialPath)) {
            try {
                ...
                this.logger.warn(`[${this.config.name}] 读取现有凭证文件失败，将覆盖:`, error);
            }
        }
