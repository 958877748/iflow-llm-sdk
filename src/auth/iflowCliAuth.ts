...
import type { IFlowOAuthCredentials, OAuthCredentials, Logger, CliAuthConfig } from '../types';
...
export class IFlowCliAuth extends BaseCliAuth {
    ...
    async getApiKey(forceRefresh = false): Promise<string | null> {
        ...
        if (forceRefresh) {
            try {
                ...
                this.logger.debug('[iFlow] 已强制刷新 access_token');
            } catch (error) {
                this.logger.warn('[iFlow] 强制刷新 access_token 失败:', error);
                return null;
            }
            // 强制刷新时清除 apiKey 缓存
        ...
        if (
            ...
            this.logger.trace(`[iFlow] 使用缓存的 apiKey (剩余 ${remainingSeconds}秒)`);
        ...
        try {
            ...
            this.logger.warn('[iFlow] 获取用户信息失败:', error);
        }
    ...
    private async fetchUserInfo(accessToken: string): Promise<{ apiKey?: string } | null> {
        try {
            ...
            const result = await response.json() as { success: boolean; data: { apiKey: string } };
            ...
            this.logger.warn('[iFlow] getUserInfo 返回数据格式异常');
            return null;
        } catch (error) {
            this.logger.error('[iFlow] 获取用户信息失败:', error);
            return null;
        }
    }
}
